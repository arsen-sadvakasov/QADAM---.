...
phase-11-search (HEAD)             = код готов, ожидает коммита
```
...
**Статус Phase 11 (2026-09-08):** код Phase 11 (Search) реализован в рабочей копии ветки `phase-11-search`, все проверки пройдены (`go build`, `go vet`, `go test ./...` — зелёные), ожидает коммита и push.
...
| 11 | Search | 🟡 код готов, не закоммичен (`phase-11-search`) | ✅ реализован |
...
**Текущий этап: Phase 11 — Search**, статус 🟡 **код реализован и протестирован локально** (build + vet + все unit-тесты зелёные), ждёт коммита и push.

Реализовано в Phase 11:
- Миграция `000007_search_indexes`: расширение `pg_trgm` + GIN-индексы на `users.full_name`, `subjects.name`, `groups.name`, `rooms.number/name` (раздел 34.2).
- Репозиторий `SearchRepository`: ILIKE-поиск по студентам/преподавателям/кураторам/группам/предметам/кабинетам с экранированием LIKE-спецсимволов (%, _, \).
- Сервис `SearchService`: права по ролям (раздел 25) — admin/curator/teacher ищут все типы; **студент — только группы/предметы/кабинеты, люди скрыты**; фильтр по типу; минимальная длина запроса 2 символа; лимит 10 результатов на тип.
- HTTP-хендлер `GET /api/v1/search?q=&type=` — все авторизованные; пустой результат отдаётся как `[]`, а не `null`.
- Роут в main.go.
- Тесты: сервис (права по ролям, фильтр типа, короткий запрос, неизвестная роль), экранирование LIKE (репозиторий), HTTP-хендлер (успех, 400, пустой массив).
...
1. Закоммитить и запушить Phase 11 в `phase-11-search` (по команде пользователя).
2. Приступить к Phase 12 (Localization): локализация KZ/RU/EN (папки `locales/`), язык в настройках пользователя.
