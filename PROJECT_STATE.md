...
phase-6-schedule-changes (HEAD)   = закоммичено и запушено
```
...
**Примечание к Phase 6 (2026-09-08):** код Phase 6 (Schedule Changes) реализован в рабочей копии ветки `phase-6-schedule-changes` и **пройден локально** (`go build`, `go vet`, `go test ./...` — все тесты зелёные), но **ещё не закоммичен и не запушен** — коммит выполняется по команде пользователя, как и для Phase 4.
...
| 6 | Schedule Changes (Замены) | 🟡 код готов, не закоммичен (`phase-6-schedule-changes`) | ✅ реализован |
...
**Текущий этап: Phase 6 — Schedule Changes (Замены)**, статус 🟡 **код реализован и протестирован локально** (build + vet + все unit-тесты зелёные), ждёт коммита и push по установленному рабочему процессу.
...
1. Закоммитить и запушить Phase 6 в `phase-6-schedule-changes` (по команде пользователя).
2. Приступить к Phase 7 (Materials): таблицы `materials`/`material_files`, HTTP API `/materials` (раздел 35), интеграция с file storage (MinIO/S3-абстракция, раздел 14).
3. Рекомендуется отдельно (не блокируя Phase 7) при наличии Docker прогнать миграции `000001`–`000004` на реальной PostgreSQL.
4. Рекомендуется решить проблему №7 (удалить неиспользуемое `JWTRefreshSecret` из конфига, либо задокументировать план использования).
